   ...
   constructor(x, y, type) {
     ...
     if (floor(random(2)) == 0) {
       ...
       for (var i = 0; i < mushroomAmount; i++) {
         ...
         var tempMushroom = new Mushroom(this.bboxX + random([-1, 1]) * 8, this.bboxY - 32 + i * 16, tempGenus, tempSpecies);
         this.mushrooms.push(tempMushroom);
...
class Log extends Objects {
   constructor(x, y, type) {
     ...
     if (floor(random(2)) == 0) {
       var mushroomAmount = floor(random(1, 6));
       var tempGenus = int(random([4, 5, 7, 8]));
       var tempSpecies = floor(random(mushroomSpecies[tempGenus].length));
       for (var i = 0; i < mushroomAmount; i++) {

        // Adds the mushroom and gives it a classification
        var tempMushroom = new Mushroom(floor(random(this.bboxX + 32, this.bboxX + this.bboxWidth - 32)), this.bboxY+ 16, tempGenus, tempSpecies);
        this.mushrooms.push(tempMushroom);
        mushrooms.push(tempMushroom);
       }
     }
...
class Stump extends Objects {
   constructor(x, y, type) {
     ...
     if (floor(random(2)) == 0) {
       ...
       // Adds the mushroom and gives it a classification
       var tempMushroom = new Mushroom(this.bboxX + this.bboxWidth/4 + random([-1, 1]) * 8, this.bboxY-8, tempGenus, tempSpecies);
       this.mushrooms.push(tempMushroom);
       mushrooms.push(tempMushroom);
       }
     }
